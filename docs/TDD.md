...
- **云存频率**: 启动登录后拉取云存；每次关键存档后同步到云端
- **冲突解决**: v0.5.4 起启用云端权威模式，云端存档存在时直接作为运行时存档；本地不再作为用户进度的持久化来源
...
- v0.3 已接入微信测试底座：
  ...
  - `CloudSaveManager` 通过 `getOpenId` 云函数获取用户标识；云端读写使用用户级 `key`（`water_sort_save_v1_cloud_<openid>`）+ `ownerOpenId` 隔离，`kv_saves` 创建者权限作为安全底线。`GAME_CONFIG.SAVE.CLOUD_AUTHORITATIVE = true` 时，云端存档是唯一权威持久化来源，本地只保留运行时内存态。
...
- 当前存档基础 key 为 `water_sort_save_v1`；微信登录成功后，云端集合 `kv_saves` 中当前用户的 `water_sort_save_v1_cloud_<openid>` 记录为权威存档。开发期若要删档测试，优先删除云数据库中当前 `ownerOpenId` / `_openid` 对应记录，再清理本地运行缓存。
